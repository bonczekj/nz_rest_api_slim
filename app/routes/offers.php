...
$app->post('/offers/update', function (Request $request, Response $response, array $args) {
    try {
        ...
        $tabOffers = new tabOffers();
        $tabOffers->update( $data['id'], $data['name'], $data['ico'], $data['processdate'], $data['processtime'], $data['deliverytype'], $data['errand'], $data['winprice'], $data['price'], $data['delivered']  );
        return $response->withHeader('Content-Type', 'application/json')
            ->withStatus(200, 'OK');
    }
    catch(Exception $e)
...
$app->post('/offers/create', function (Request $request, Response $response, array $args) {
    ...
    $data = json_decode($json, true); // parse the JSON into an assoc. array
    $tabOffers = new tabOffers();
        $tabOffers->insert( $data['id'], $data['name'], $data['ico'], $data['processdate'], $data['processtime'], $data['deliverytype'], $data['errand'], $data['winprice'], $data['price'], $data['delivered'] );
        return $response->withHeader('Content-Type', 'application/json')
                        ->withStatus(200, 'OK');
    }
    catch(Exception $e)
